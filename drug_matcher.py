...
class DrugMatcher:
    ...
    def load_data(self, raw_file_path, sfda_file_path, chi_file_path):
        """
        ...
        print(f"  → SFDA Columns: {self.sfda_ref.columns.tolist()}")
        
        # Load CHI reference
        self.chi_ref = pd.read_excel(chi_file_path)
        print(f"✓ Loaded {len(self.chi_ref):,} rows from CHI reference")
        print(f"  → CHI Columns: {self.chi_ref.columns.tolist()}\n")
    ...
    def _display_data_info(self):
        ...
        print("="*60)
        print("DRUG DATA OVERVIEW")
        print("="*60)
        
        print("\nRaw Data Sample:")
        if 'service_code' in self.raw_data.columns:
            sample_codes = self.raw_data['service_code'].dropna().head(3).tolist()
            print(f"  Sample service_codes: {sample_codes}")
        
        print("\nSFDA Reference Sample:")
        if 'RegisterNumber' in self.sfda_ref.columns:
            sample_reg = self.sfda_ref['RegisterNumber'].dropna().head(3).tolist()
            print(f"  Sample RegisterNumbers: {sample_reg}")
        if 'Scientific Name' in self.sfda_ref.columns:
            sample_sci = self.sfda_ref['Scientific Name'].dropna().head(3).tolist()
            print(f"  Sample Scientific Names: {sample_sci}")
        
        print("\nCHI Reference Sample:")
        if 'SCIENTIFIC NAME' in self.chi_ref.columns:
            sample_chi_sci = self.chi_ref['SCIENTIFIC NAME'].dropna().head(3).tolist()
            print(f"  Sample Scientific Names: {sample_chi_sci}")
        print()
    ...
    def match_sfda(self):
        ...
        Retrieve Scientific Name from SFDA
        """
        print("="*60)
    ...
    def match_chi(self):
        ...
        Retrieve ICD10 Code, INDICATION, and NOTES from CHI
        """
        print("="*60)
    ...
    def run_pipeline(self, raw_file, sfda_file, chi_file,
        ...
        self.load_data(raw_file, sfda_file, chi_file)
...
if __name__ == "__main__":
    ...
    results = matcher.run_pipeline(
        raw_file='raw_claims.xlsx',
        sfda_file='data/sfda/SFDA 10-2025.xls',
        chi_file='data/chi/CHI Drug Formulary Compilation and Formatting 08-2025.xlsx',
    ...
    print("Drug matching pipeline completed successfully!")
